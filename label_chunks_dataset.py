#!/usr/bin/env python3
"""
Label the pages_5_38_chunks.json dataset using the embodiment extractor.
This script will:
1. Extract abstract, summary, and claims from patent.json
2. Generate invention principles from these sections
3. Label each chunk as embodiment/non-embodiment
4. Map chunks to relevant principles and claims
"""
...
async def process_chunks_dataset():
    ...
    for i, principle in enumerate(principles):
        print(f"  P{i+1}: {principle.text[:100]}...")
    
    # Filter chunks that might be embodiments
    print("Filtering potential embodiment chunks...")
    chunk_texts = [chunk['text'] for chunk in chunks_data]
    filtered_texts = filter_candidates(chunk_texts)
    print(f"Filtered to {len(filtered_texts)} potential embodiment chunks")
    
    # Create mapping from text back to original chunk data
    text_to_chunk = {chunk['text']: chunk for chunk in chunks_data}
    
    # Classify each filtered chunk
    print("Classifying chunks...")
    labeled_chunks = []
    
    for i, chunk_text in enumerate(filtered_texts):
        print(f"Processing chunk {i+1}/{len(filtered_texts)}...")
        
        # Get the original chunk data
        original_chunk = text_to_chunk[chunk_text]
        ...
        labeled_chunk = {
            ...
            'char_count': original_chunk['char_count'],
            'content_type': original_chunk['content_type'],
            ...
            'processing_timestamp': '2025-06-22T20:06:44-04:00'
        }
...
def save_results(labeled_chunks: List[Dict], principles: List[Principle]):
    ...
    print(f"\nResults saved:")
    print(f"  Labeled chunks (JSON): {output_file}")
    print(f"  Labeled chunks (JSONL): {jsonl_file}")
    print(f"  Invention principles: {principles_file}")
...
def analyze_results(labeled_chunks: List[Dict]):
    ...
    print(f"Embodiment chunks: {embodiment_chunks} ({embodiment_chunks/total_chunks*100:.1f}%)")
    print(f"Non-embodiment chunks: {non_embodiment_chunks} ({non_embodiment_chunks/total_chunks*100:.1f}%)")
    
    print(f"\nPrinciple mapping frequency:")
    for principle, count in sorted(principle_counts.items()):
        print(f"  {principle}: {count} chunks")
    
    print(f"\nClaim mapping frequency:")
    for claim, count in sorted(claim_counts.items()):
        print(f"  Claim {claim}: {count} chunks")
    ...
    for i, chunk in enumerate(embodiments[:3]):
        ...
        print(f"  Principles: {chunk['mapped_principles']}")
        print(f"  Claims: {chunk['mapped_claims']}")
        print(f"  Text: {chunk['text'][:200]}...")
        print(f"  Justification: {chunk['justification']}")


async def main():
    """Main function."""
    print("Starting embodiment extraction pipeline...")
    ...
    try:
        # Process the dataset
        labeled_chunks, principles = await process_chunks_dataset()
